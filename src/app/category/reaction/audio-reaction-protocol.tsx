...
export function AudioReactionProtocol({ initialAttempts, initialBestScore, isSignedIn }: AudioReactionProtocolProps) {
  ...
  // Auto-start first round in duel mode
  useEffect(() => {
    if (!isMultiplayerSession || roundTimes.length > 0) return;
    const t = setTimeout(() => startProtocol(), 100);
  ...
  const arenaTone =
    ...
    : phase === 'clicked' ? 'border-cyan-300 bg-cyan-100 text-slate-900'
  ...
  const arenaTitle =
    ...
    : phase === 'waiting' || phase === 'ready' ? 'Listen'
    ...
    <section className="lab-card p-4 sm:p-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-800 sm:text-3xl">Audio Reaction</h2>
        </div>
      ...
      >
        ...
        <span className="mt-4 max-w-md text-sm font-bold uppercase tracking-[0.18em] sm:text-base">
          {phase === 'too-soon' ? 'Click to restart.' :
           ...
           phase === 'waiting' || phase === 'ready' ? 'Wait for the beep and react as fast as possible.' :
