...
export function MultiReactionProtocol({ initialAttempts, isSignedIn }: { initialAttempts: number; initialBestScore: number | null; isSignedIn: boolean }) {
  ...
  return (<section className="lab-card p-4 sm:p-6">
    ...
    <div className="relative">
      ...
      <div className={`grid grid-cols-2 gap-4 ${phase==='too-soon'?'opacity-60':''}`}>{LABELS.map((l,i)=>{const a=phase==='active'&&activeBtn===i; return <button key={i} className={`relative flex min-h-[var(--panel-h-xs)] flex-col items-center justify-center rounded-[2rem] border-2 px-4 py-6 text-center transition-all duration-75 sm:min-h-[var(--panel-h-xs-md)] cursor-pointer ${a?ACTIVE_STY[i]:IDLE_STY[i]}`} onPointerDown={()=>click(i)} type="button">{phase==='too-soon'?<><span className="text-3xl font-black tracking-tight text-rose-600 sm:text-4xl">Too soon</span><span className="mt-2 text-[10px] font-bold uppercase tracking-[0.18em] text-rose-500">CLICK TO RESTART</span></>:<><span className="text-5xl font-black tracking-tight sm:text-6xl">{l}</span>{isIdle && <span className="mt-2 text-xs font-bold uppercase tracking-[0.18em] opacity-60">{phase==='idle'?'Click to start':'Click to restart'}</span>}</>}</button>})}</div>
    </div>
