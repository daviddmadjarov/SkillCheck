...
export function MultiReactionProtocol({ initialAttempts, isSignedIn }: { initialAttempts: number; initialBestScore: number | null; isSignedIn: boolean }) {
  ...
  return (<section className="lab-card flex flex-col min-h-dvh p-4 sm:p-6">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Reaction Category</p><h2 className="mt-2 text-2xl font-black tracking-tight text-slate-800 sm:text-3xl">Multi-Reaction</h2></div><div className="flex flex-wrap items-center gap-2">{!isIdle && <div className="rounded-full border-2 border-indigo-200 bg-indigo-50 px-3 py-1.5 text-sm font-bold text-indigo-700">Round {Math.min(curRound+1,NUM_BUTTONS)} / {NUM_BUTTONS}</div>}<div className="rounded-full border-2 border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-600">{isSignedIn?'Leaderboard sync active':'Guest mode'}</div></div></div>
    <div className="flex-1 flex flex-col justify-center"><div className="relative">
      ...
      <div className={`grid grid-cols-2 gap-4 ${phase==='too-soon'?'opacity-60':''}`}>{LABELS.map((l,i)=>{const a=phase==='active'&&activeBtn===i; return <button key={i} className={`relative flex min-h-[10rem] flex-col items-center justify-center rounded-[2rem] border-2 px-4 py-6 text-center transition-all duration-75 sm:min-h-[12rem] cursor-pointer ${a?ACTIVE_STY[i]:IDLE_STY[i]}`} onPointerDown={()=>click(i)} type="button">{phase==='too-soon'?<><span className="text-3xl font-black tracking-tight text-rose-600 sm:text-4xl">Too soon</span><span className="mt-2 text-[10px] font-bold uppercase tracking-[0.18em] text-rose-500">CLICK TO RESTART</span></>:<><span className="text-5xl font-black tracking-tight sm:text-6xl">{l}</span>{isIdle && <span className="mt-2 text-xs font-bold uppercase tracking-[0.18em] opacity-60">{phase==='idle'?'Click to start':'Click to restart'}</span>}</>}</button>})}</div>
    </div>
