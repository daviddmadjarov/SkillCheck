...
import { LogOut, MoonStar, Sparkles, SunMedium, UserCircle2 } from 'lucide-react';
...
export function ProfilePanel({ displayName, email, profileMessage, username }: ProfilePanelProps) {
  ...
  return (
    <div className={`space-y-4 rounded-[1.6rem] border-2 p-4 ${isDarkMode ? 'border-slate-700 bg-slate-950 text-slate-100 shadow-[0_10px_0_rgba(15,23,42,0.65)]' : 'border-cyan-200 bg-cyan-50 shadow-[0_10px_0_rgba(186,230,253,0.8)]'}`}>
      ...
      <div className={`rounded-[1.4rem] border-2 p-4 ${isDarkMode ? 'border-slate-700 bg-slate-950' : 'border-slate-200 bg-slate-50'}`}>
        <div className="flex items-center gap-2 text-slate-700">
          <Sparkles className={`h-4 w-4 ${isDarkMode ? 'text-cyan-300' : 'text-amber-500'}`} />
          <span className={`text-sm font-bold ${isDarkMode ? 'text-slate-100' : 'text-slate-700'}`}>Profile</span>
        </div>
        <p className={`mt-2 text-sm font-medium leading-6 ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
          Public name, session and leaderboard identity.
        </p>
      </div>
